...
void PlayGraph::update()
{
	...
	if (onCountDown_)
	{
		...
		{
			//簡単なゲーム説明
			DrawStringToHandle(explane_rule_position.x, explane_rule_position.y, "HPを0にせずに", GetColor(255, 50, 50), fontHandle_);
		...
		{
			DrawStringToHandle(explane_rule_position.x - 130, explane_rule_position.y, "相手を落として生き残れ!", GetColor(255, 50, 50), fontHandle_);
